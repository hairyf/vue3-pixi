...
let count = 0
const spriteRef = ref<SpriteElement>()
const visible = ref(false)
const maggots = reactive<DudeIte[]>([])
const position = reactive({ x: 0, y: 0 })

const bounds = ref(new Rectangle())

watch(screen, (screen) => {
  bounds.value.x = -padding
  bounds.value.y = -padding
  bounds.value.width = screen.width + padding * 2
  bounds.value.height = screen.height + padding * 2
}, { immediate: true })
...
<template>
  <assets
    :entry="[
      ...
      { alias: 'displace', src: 'https://pixijs.com/assets/pixi-filters/displace.png' },
      { alias: 'ring', src: 'https://pixijs.com/assets/pixi-filters/ring.png' },
      { alias: 'maggot_tiny', src: 'https://pixijs.com/assets/maggot_tiny.png' },
    ]"
  >
    <sprite
      texture="bg_grass"
