...
export function patchSpriteProps(el: Sprite, key: string, prevValue: any, nextValue: any): boolean {
  ...
  if (key.startsWith('anchor'))
    return setPointValue('anchor', el, key, prevValue, nextValue)
  return false
}
...
export function patchDefaultProps(el: Container, key: string, prevValue: any, nextValue: any) {
  if (key.startsWith('position'))
    return setPointValue('position', el, key, prevValue, nextValue)
  if (key.startsWith('scale'))
    return setPointValue('scale', el, key, prevValue, nextValue)
  if (key.startsWith('pivot'))
    return setPointValue('pivot', el, key, prevValue, nextValue)
  return false
}
